...
QMyThreadShow::QMyThreadShow()
{

}

void QMyThreadShow::run()
{
    m_Stop=false;//启动线程时令m_stop=false
    ...
    while(!m_Stop)//循环主体
    {
        ...
        emit    newValue(bufferData,BufferSize,sec);//给主线程传递数据
    }
    quit();
}

bool QMyThreadShow::Stop() const
{
    return m_Stop;
}

void QMyThreadShow::setStop(bool newStop)
{
    m_Stop = newStop;
}
